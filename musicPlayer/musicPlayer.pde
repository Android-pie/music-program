...
AudioMetaData[] songMetaData = new AudioMetaData[numbOfSongs];//needs to be an array
...
void setup() {
  ...
  for (int i=currentSong; i<numbOfSongs; ) {
    ...
    println("Song lenght (in milliseconds); ", songMetaData[currentSong].length() );
    println("Song Lenght (in second): ", songMetaData[currentSong].length()/1000);
